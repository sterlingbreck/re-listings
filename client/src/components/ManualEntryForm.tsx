...
export function ManualEntryForm({ url, initial, scrapeError, onCancel, onSubmit }: Props) {
  ...
  return (
    ...
    >
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-black/10 bg-white p-6 shadow-2xl sm:rounded-2xl">
        <h2 id="manual-form-title" className="text-lg font-bold text-black">
          Fill in listing details
        </h2>
        <p className="mt-1 text-sm text-neutral-500">
          {scrapeError
            ? `Couldn't auto-fetch this listing (${scrapeError}). Enter the details manually.`
            : 'Some details were missing — please complete the listing.'}
        </p>
        <p className="mt-2 truncate text-xs text-neutral-400" title={url}>
          {url}
        </p>
